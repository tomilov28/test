...
## Phase 5 - audit remediation and operational validation (current)

Completed on baseline `425849e`. All items below are backed by live runs in this
tree (JUnit XML + scenario logs + evidence JSON regenerated by the Makefile
targets); `PROGRESS.md` itself is a summary, not evidence.

### Part A - audit findings A01-A12

- [x] A01 domain-first cancellation: Request `CLOSED/CANCELLED` committed in the
      same transaction that enqueues `CANCEL_PROCESS`; dispatcher converges the
      engine instance idempotently; reconciler never closes a Request
- [x] A01a engine-down cancel (immediate outcome, async convergence),
      A01b exhausted retries + `/admin/commands/{id}/requeue` (outcome untouched),
      A01c cancel-before-start (no orphan) - scenario PASS for both engines
- [x] A02 domain-first completion: `CompletionContract` validated at the API;
      dispatcher never re-decides; engine END is a technical state
- [x] A02a engine-down completion + requeue, A02b invalid contract rejected,
      A02c engine-ended-without-outcome recorded as anomaly, Request stays ACTIVE
      - scenario PASS for both engines
- [x] A03 outbox lease from `processing_started_at` - 4/4 PostgreSQL tests
      (`tests/test_outbox_lease.py`)
- [x] A04 idempotent convergence (query-before-start, task-exists, instance-ENDED)
- [x] A05 fault controller `remaining==0` never fires (`tests/test_fault_injector.py`)
- [x] A06 engine-specific REST collectors (Flowable `/service` prefix fixed)
- [x] A07 reproducibility: `fault-operaton` / `fault-flowable` rerun suites +
      scenarios, save JUnit + logs; fixture redeployed between suite and scenarios
- [x] A08 down engine FAILS the run (never a silent skip)
- [x] A09 `make reset-state` authoritative (drop volume, recreate engine DBs)
- [x] A10 shared app pid/log; fingerprint-only process discovery; no leftover procs
- [x] A11 Operaton example module disabled via official config (REST+Cockpit kept)
- [x] A12 image digest pinning recorded in `artifacts/*/resources.json`

### Part A verification (live)

| Run | Result |
| --- | ------ |
| `make fault-operaton` | integration suite 22/22 PASS; scenarios R1-R5, stress, a01a-a02c: 12/12 PASS |
| `make fault-flowable` | integration suite 23/23 PASS; scenarios R1-R5, stress, deadlock (0/25 recurrence), a01a-a02c: 13/13 PASS |
| `make test-unit` | 37 passed, 35 deselected |

Evidence: `artifacts/{operaton,flowable}/fault-test-results.xml`,
`fault-suite.log`, `fault-scenarios.log`, `faults/*.json`,
`api-evidence/*.json`. Details: `AUDIT_REMEDIATION.md`.

### Part B - resource measurement (clean-DB protocol)

- [x] `make measure-resources` measured both engines with ONLY postgres + the
      target engine running (app + workers stopped), 60s settle, 6 samples
- [x] Parity enforced: both engines `cpus=2`, `mem_limit=1024m`; verdict
      `STRICTLY COMPARABLE` for both
- [x] Operaton 2.1.4: median RSS 348.4 MiB, median CPU 0.2%
- [x] Flowable 8.0.0: median RSS 285.0 MiB, median CPU 0.3%
- [x] Evidence: `artifacts/{operaton,flowable}/resources.json` (samples,
      container/image ids, repo digests). Details: `RESOURCE_METHODOLOGY.md`.

### Part C - operations demo + incident

- [x] Operaton demo PASS (request -> parallel tasks -> timer -> APPROVE,
      outcome COMPLETED); incident PASS (5 REST actions for diagnosis,
      recovery `PUT /external-task/{id}/retries {retries:1}`)
- [x] Flowable demo PASS; incident PASS (4 REST actions for diagnosis,
      recovery `POST /service/management/deadletter-jobs/{id} {action:move}`)
- [x] Evidence: `artifacts/{operaton,flowable}/faults/ops_demo_{demo,incident}.json`.
      Details: `OPERATIONS_COMPARISON.md`, `MANUAL_DEMO.md`.

### Final clean-state verification

- [x] `make reset-state` (migration chain `298bb5dfa5ff -> e1a2b3c4d5e6`,
      engine DBs empty, no stale processes)
- [x] `make test-unit` 37 passed; `make test-operaton` (22 integration,
      T07/T09 PASS) / `make test-flowable` (23 integration, F07/F09 PASS)
      rerun on the clean DB (JUnit: `artifacts/*/test-results.xml`)
- [x] Fresh-DB schema bootstrap fixed: `OPERATON_BPM_DATABASE_SCHEMA_UPDATE=true`
      (`docker-compose.operaton.yml`) makes the Operaton engine create its
      schema idempotently after `make reset-state` drops the PG volume
      (verified: 49 `act_*` tables on a freshly recreated `operaton` DB).
- [x] `wait_for_engine.py` FLOWABLE probe path corrected
      (`/repository/process-definitions`, the base URL already ends in `/service`).
